...
start-welcome = 
    ...
    I am a bot AI for football predictions ⚽
    🎯 What I can do:
...
match-league = League: { $competition }

stats-title = Match Statistics
stats-form = Form: { $form }
...
stats-home-title = Home Stats: { $team }
stats-away-title = Away Stats: { $team }
stats-goals-for = Scored: { $total } ({ $avg } avg)
stats-goals-against = Conceded: { $total } ({ $avg } avg)
stats-avg = avg
stats-wins-draws-losses = Wins: { $wins } | Draws: { $draws } | Losses: { $losses }
stats-h2h-not-found = No H2H history found
stats-h2h-recent = Recent encounters:
stats-avg-goals = Avg goals: { $avg }
...
predict-insufficient = Insufficient stats for { $type }.
...
predict-goals-under = Total Under { $val }:
